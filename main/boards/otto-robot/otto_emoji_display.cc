...
OttoEmojiDisplay::OttoEmojiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    : SpiLcdDisplay(panel_io, panel, width, height, offset_x, offset_y, mirror_x, mirror_y, swap_xy),
      emotion_gif_(nullptr), use_otto_emoji_(true) {
    SetupGifContainer();
};
        ...
        if (emotion_gif_) {
            for (const auto& map : emotion_maps_) {
                if (map.name && strcmp(map.name, emotion) == 0) {
                    ...
                    ESP_LOGI(TAG, "设置Otto表情: %s", emotion);
                    return;
                }
            }
            lv_gif_set_src(emotion_gif_, &staticstate);
            ESP_LOGI(TAG, "未知Otto表情'%s'，使用默认", emotion);
        }
    } else {
        // 使用默认emoji显示系统
        SpiLcdDisplay::SetEmotion(emotion);
        ESP_LOGI(TAG, "设置默认表情: %s", emotion);
    }
}
