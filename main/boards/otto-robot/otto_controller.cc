...
public:
    ...
    void RegisterMcpTools() {
        ...
        mcp_server.AddTool("self.display.set_emoji_mode", "切换表情显示模式", 
                           PropertyList({Property("mode", kPropertyTypeString)}),
                           [](const PropertyList& properties) -> ReturnValue {
                               auto mode = properties["mode"].value<std::string>();
                               auto& board = Board::GetInstance();
                               auto* display = dynamic_cast<OttoEmojiDisplay*>(board.GetDisplay());
                               
                               if (!display) {
                                   return "error: display is not OttoEmojiDisplay";
                               }
                               
                               if (mode == "otto") {
                                   display->SetEmojiMode(true);
                                   return "switched to Otto emoji mode";
                               } else if (mode == "default") {
                                   display->SetEmojiMode(false);
                                   return "switched to default emoji mode";
                               } else {
                                   return "error: mode must be 'otto' or 'default'";
                               }
                           });

        mcp_server.AddTool("self.display.get_emoji_mode", "获取当前表情显示模式", PropertyList(),
                           [](const PropertyList& properties) -> ReturnValue {
                               auto& board = Board::GetInstance();
                               auto* display = dynamic_cast<OttoEmojiDisplay*>(board.GetDisplay());
                               ...
                               return display->IsUsingOttoEmoji() ? "otto" : "default";
                           });
