...
static esp_err_t StartKikiWebServer() {
    return otto_start_webserver();
}

class KikiRobot : public WifiBoard {
private:
    LcdDisplay* display_;
    ...
    void InitializeWebServer() {
        ESP_LOGI(TAG, "Initializing Kiki Web Controller");
        ...
        xTaskCreate([](void* param) {
            ...
            if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
                ESP_LOGI(TAG, "✅ WiFi connected, starting Kiki Web Controller...");
                StartKikiWebServer();
                ESP_LOGI(TAG, "🌐 Kiki Web Controller available - No password required!");
            } else {
                ESP_LOGI(TAG, "⏳ WiFi not connected yet - Kiki Web Controller will retry...");
        ...
        state_manager.RegisterStateChangeCallback(
            [this](DeviceState previous_state, DeviceState current_state) {
                ...
                if (previous_state == kDeviceStateListening && 
                    ...
                    ESP_LOGW(TAG, "❌ ASR error detected - Robot will scratch");
...
public:
    KikiRobot() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeSpi();
    ...
    void DisplayStationIP() {
        ...
        if (!wifi_station.IsConnected()) {
            ...
            if (display_) {
                ...
                if (otto_display) {
                    otto_display->SetChatMessage("system", "WiFi chưa kết nối!");
                }
            }
            return;
        }
        ...
        if (netif && esp_netif_get_ip_info(netif, &ip_info) == ESP_OK) {
            ...
            if (display_) {
                auto* otto_display = static_cast<OttoEmojiDisplay*>(display_);
                if (otto_display) {
                    otto_display->SetChatMessage("system", ip_str);
                }
            }
        } else {
            ...
            if (display_) {
                auto* otto_display = static_cast<OttoEmojiDisplay*>(display_);
                if (otto_display) {
                    otto_display->SetChatMessage("system", "Không thể lấy IP!");
                }
            }
        }
    }
...
DECLARE_BOARD(KikiRobot);

// C interface for touch sensor control
extern "C" {
    void otto_set_touch_sensor_enabled(bool enabled) {
        auto& board = Board::GetInstance();
        auto kiki_board = dynamic_cast<KikiRobot*>(&board);
        if (kiki_board) {
            kiki_board->SetTouchSensorEnabled(enabled);
        }
    }
    ...
    bool otto_is_touch_sensor_enabled() {
        ...
        auto kiki_board = dynamic_cast<KikiRobot*>(&board);
        if (kiki_board) {
            return kiki_board->IsTouchSensorEnabled();
        }
        return false;
    }
}
