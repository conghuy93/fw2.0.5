...
public:
    ...
    void RegisterMcpTools() {
        ...
        mcp_server.AddTool("self.dog.pushup",
                           "🐕 I do pushups like exercising! I push up and down with my front legs!\n"
                           "Args:\n"
                           "  pushups (1-10): How many pushups to do\n"
                           "  speed (100-500ms): Pushup speed\n"
                           "Example: 'Otto, do pushups!' or 'Exercise time!' or 'Show me your strength!'",
                           PropertyList({Property("pushups", kPropertyTypeInteger, 3, 1, 10),
                                         Property("speed", kPropertyTypeInteger, 150, 100, 500)}),
                           [this](const PropertyList& properties) -> ReturnValue {
                               int pushups = properties["pushups"].value<int>();
                               int speed = properties["speed"].value<int>();
                               ESP_LOGI(TAG, "💪 Kiki is doing pushups!");
                               otto_.DogPushup(pushups, speed);
                               return true;
                           });

        mcp_server.AddTool("self.dog.balance",
                           "🐕 I balance on my hind legs like a circus dog! I stand up on two back legs!\n"
                           "Args:\n"
                           "  duration_ms (500-5000): How long to hold balance in milliseconds\n"
                           "  speed (100-500ms): Balance movement speed\n"
                           "Example: 'Otto, balance!' or 'Stand on hind legs!' or 'Two leg stand!'",
                           PropertyList({Property("duration_ms", kPropertyTypeInteger, 2000, 500, 5000),
                                         Property("speed", kPropertyTypeInteger, 150, 100, 500)}),
                           [this](const PropertyList& properties) -> ReturnValue {
                               int duration_ms = properties["duration_ms"].value<int>();
                               int speed = properties["speed"].value<int>();
                               ESP_LOGI(TAG, "⚖️ Kiki is balancing!");
                               otto_.DogBalance(duration_ms, speed);
                               return true;
                           });
