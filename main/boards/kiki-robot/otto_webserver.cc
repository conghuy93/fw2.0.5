...
// UDP Drawing Service - global pointers for web UI access
...
void send_otto_control_page(httpd_req_t *req) {
    ...
    httpd_resp_sendstr_chunk(req, "<button class='btn' onclick='sendAction(\"dog_balance\", 2000, 150)'>⚖️ Đứng 2 Chân</button>");
    httpd_resp_sendstr_chunk(req, "</div>");
    httpd_resp_sendstr_chunk(req, "</div>");
    ...
    // Volume Control Section - MOVED TO TOP
...
esp_err_t otto_volume_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (board.GetDisplay()) {
            ...
            auto* otto_display = dynamic_cast<OttoEmojiDisplay*>(board.GetDisplay());
            if (otto_display) {
                otto_display->SetChatMessage("system", volume_msg);
            }
        }
...
// Drawing mode handler - Enable/disable drawing canvas
esp_err_t otto_drawing_mode_handler(httpd_req_t *req) {
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    
    if (!g_udp_draw_service) {
        httpd_resp_set_status(req, "503 Service Unavailable");
        httpd_resp_sendstr(req, "{\"error\":\"UDP Drawing Service not initialized\"}");
        return ESP_OK;
    }
    
    // Parse query parameter: enable=true/false
    char query[100];
    bool enable = false;
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (httpd_query_key_value(query, "enable", enable_str, sizeof(enable_str)) == ESP_OK) {
            enable = (strcmp(enable_str, "true") == 0 || strcmp(enable_str, "1") == 0);
        }
    }
    
    // Enable/disable drawing mode
    g_udp_draw_service->EnableDrawingMode(enable);
    
    // Send JSON response
    httpd_resp_set_type(req, "application/json");
    char response[200];
    snprintf(response, sizeof(response), 
            "{\"success\":true,\"drawing_mode\":%s}", 
            enable ? "true" : "false");
    httpd_resp_sendstr(req, response);
    
    ESP_LOGI(TAG, "🎨 Drawing mode: %s", enable ? "ON" : "OFF");
    return ESP_OK;
}
...
esp_err_t otto_drawing_clear_handler(httpd_req_t *req) {
    ...
    if (!g_udp_draw_service) {
        httpd_resp_set_status(req, "503 Service Unavailable");
        httpd_resp_sendstr(req, "UDP Drawing Service not initialized");
        return ESP_OK;
    }
    
    g_udp_draw_service->ClearCanvas();
    
    httpd_resp_set_type(req, "application/json");
    httpd_resp_sendstr(req, "{\"success\":true,\"message\":\"Canvas cleared\"}");
    
    ESP_LOGI(TAG, "🧹 Canvas cleared via web");
    return ESP_OK;
}

// Draw pixel handler - For web canvas drawing
esp_err_t otto_drawing_pixel_handler(httpd_req_t *req) {
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    
    if (!g_drawing_display) {
        ESP_LOGW(TAG, "⚠️ Drawing display not initialized!");
        httpd_resp_set_status(req, "503 Service Unavailable");
        httpd_resp_sendstr(req, "Drawing Display not initialized");
        return ESP_OK;
    }
    
    // Parse query: x, y, state
    char query[100];
    int x = -1, y = -1, state = 1;
    ...
    // Debug logging
    static int pixel_request_count = 0;
    if (pixel_request_count++ < 5) {
        ESP_LOGI(TAG, "🎨 Pixel request: x=%d, y=%d, state=%d, canvas_enabled=%d", 
                 x, y, state, g_drawing_display->IsCanvasEnabled());
    }
    
    // Validate and draw
    if (x >= 0 && x < g_drawing_display->GetWidth() && 
        y >= 0 && y < g_drawing_display->GetHeight()) {
        if (g_drawing_display->IsCanvasEnabled()) {
            g_drawing_display->DrawPixel(x, y, state != 0);
            httpd_resp_set_type(req, "application/json");
            httpd_resp_sendstr(req, "{\"success\":true}");
        } else {
            ESP_LOGW(TAG, "⚠️ Canvas not enabled! Cannot draw pixel");
            httpd_resp_set_status(req, "400 Bad Request");
            httpd_resp_sendstr(req, "{\"error\":\"Drawing mode not enabled\"}");
        }
    } else {
        ESP_LOGW(TAG, "⚠️ Invalid coordinates: x=%d, y=%d (max: %dx%d)", 
                 x, y, g_drawing_display->GetWidth(), g_drawing_display->GetHeight());
        httpd_resp_set_status(req, "400 Bad Request");
        httpd_resp_sendstr(req, "{\"error\":\"Invalid coordinates\"}");
    }
    
    return ESP_OK;
}

// Drawing status handler - Get current status
esp_err_t otto_drawing_status_handler(httpd_req_t *req) {
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_set_type(req, "application/json");
    
    if (!g_udp_draw_service) {
        httpd_resp_sendstr(req, "{\"available\":false,\"error\":\"Service not initialized\"}");
        return ESP_OK;
    }
    
    auto stats = g_udp_draw_service->GetStats();
    
    // Check drawing mode from DrawingDisplay, not OttoEmojiDisplay
    bool drawing_enabled = g_drawing_display ? g_drawing_display->IsCanvasEnabled() : false;
    int width = g_drawing_display ? g_drawing_display->GetWidth() : 240;
    int height = g_drawing_display ? g_drawing_display->GetHeight() : 280;
    ...
    snprintf(response, sizeof(response),
            ...
            "\"errors\":%lu"
            "}",
            g_udp_draw_service->IsRunning() ? "true" : "false",
            drawing_enabled ? "true" : "false",
            width,
            height,
            (unsigned long)stats.packets_received,
            (unsigned long)stats.packets_processed,
            (unsigned long)stats.pixels_drawn,
...
// Drawing page handler - Serve HTML5 canvas drawing interface
esp_err_t otto_drawing_page_handler(httpd_req_t *req) {
    const char* html_page = 
        ...
        "<title>Otto Drawing Canvas 🎨</title>"
        "<style>"
        "body{font-family:Arial,sans-serif;margin:0;padding:20px;background:#1a1a1a;color:#fff;text-align:center}"
        "h1{margin:10px 0;color:#4CAF50}"
        ".container{max-width:800px;margin:0 auto}"
        ".status{background:#2a2a2a;padding:10px;border-radius:5px;margin:10px 0}"
        ".controls{margin:15px 0}"
        "button{background:#4CAF50;color:white;border:none;padding:12px 24px;font-size:16px;margin:5px;border-radius:5px;cursor:pointer}"
        "button:hover{background:#45a049}"
        "button:disabled{background:#666;cursor:not-allowed}"
        ".btn-danger{background:#f44336}"
        ".btn-danger:hover{background:#da190b}"
        "#drawCanvas{border:2px solid #4CAF50;background:#000;cursor:crosshair;touch-action:none;margin:10px auto;display:block}"
        ".info{font-size:14px;color:#999;margin:10px 0}"
        ".stats{display:flex;justify-content:space-around;flex-wrap:wrap}"
        ".stat{margin:5px;padding:5px 10px;background:#333;border-radius:3px}"
        "</style>"
        "</head><body>"
        "<div class='container'>"
        "<h1>🎨 Otto Drawing Canvas</h1>"
        "<div class='status' id='status'>Loading...</div>"
        "<div class='controls'>"
        "<button onclick='toggleDrawing()' id='toggleBtn'>Enable Drawing</button>"
        "<button onclick='clearCanvas()' class='btn-danger'>Clear Canvas</button>"
        "</div>"
        "<canvas id='drawCanvas' width='240' height='280'></canvas>"
        "<div class='info'>Draw with mouse/touch • Black = erase</div>"
        "<div class='stats' id='stats'></div>"
        "</div>"
        "<script>"
        "const canvas=document.getElementById('drawCanvas');"
        "const ctx=canvas.getContext('2d');"
        "let drawing=false,drawingMode=false,lastX=0,lastY=0;"
        "ctx.fillStyle='#000';ctx.fillRect(0,0,240,280);"
        "function updateStatus(){"
        "fetch('/drawing_status').then(r=>r.json()).then(d=>{"
        "document.getElementById('status').innerHTML="
        "`🟢 UDP: ${d.running?'ON':'OFF'} | 🎨 Drawing: ${d.drawing_mode?'ENABLED':'DISABLED'} | 📊 ${d.width}x${d.height}px`;"
        "document.getElementById('stats').innerHTML="
        "`<div class='stat'>📦 Received: ${d.packets_received}</div>`+"
        "`<div class='stat'>✅ Processed: ${d.packets_processed}</div>`+"
        "`<div class='stat'>🎨 Pixels: ${d.pixels_drawn}</div>`+"
        "`<div class='stat'>❌ Errors: ${d.errors}</div>`;"
        "drawingMode=d.drawing_mode;"
        "document.getElementById('toggleBtn').textContent=drawingMode?'Disable Drawing':'Enable Drawing';"
        "}).catch(e=>document.getElementById('status').innerHTML='❌ Error: '+e);}"
        "function toggleDrawing(){"
        "fetch(`/drawing_mode?enable=${!drawingMode}`).then(r=>r.json()).then(d=>{"
        "drawingMode=d.drawing_mode;updateStatus();"
        "}).catch(alert);}"
        "function clearCanvas(){"
        "if(confirm('Clear canvas?')){"
        "fetch('/drawing_clear').then(()=>{ctx.fillStyle='#000';ctx.fillRect(0,0,240,280);updateStatus();}).catch(alert);}}"
        "function draw(x,y,state){"
        "if(!drawingMode)return;"
        "ctx.fillStyle=state?'#fff':'#000';"
        "ctx.fillRect(x,y,1,1);"
        "fetch(`/drawing_pixel?x=${x}&y=${y}&state=${state?1:0}`).catch(console.error);}"
        "canvas.addEventListener('mousedown',e=>{drawing=true;const rect=canvas.getBoundingClientRect();"
        "lastX=Math.floor((e.clientX-rect.left)*240/rect.width);"
        "lastY=Math.floor((e.clientY-rect.top)*280/rect.height);"
        "draw(lastX,lastY,e.buttons===1);});"
        "canvas.addEventListener('mousemove',e=>{if(!drawing)return;const rect=canvas.getBoundingClientRect();"
        "const x=Math.floor((e.clientX-rect.left)*240/rect.width);"
        "const y=Math.floor((e.clientY-rect.top)*280/rect.height);"
        "if(x!==lastX||y!==lastY){draw(x,y,e.buttons===1);lastX=x;lastY=y;}});"
        "canvas.addEventListener('mouseup',()=>drawing=false);"
        "canvas.addEventListener('touchstart',e=>{e.preventDefault();drawing=true;const rect=canvas.getBoundingClientRect();"
        "const touch=e.touches[0];lastX=Math.floor((touch.clientX-rect.left)*240/rect.width);"
        "lastY=Math.floor((touch.clientY-rect.top)*280/rect.height);draw(lastX,lastY,true);});"
        "canvas.addEventListener('touchmove',e=>{e.preventDefault();if(!drawing)return;const rect=canvas.getBoundingClientRect();"
        "const touch=e.touches[0];const x=Math.floor((touch.clientX-rect.left)*240/rect.width);"
        "const y=Math.floor((touch.clientY-rect.top)*280/rect.height);"
        "if(x!==lastX||y!==lastY){draw(x,y,true);lastX=x;lastY=y;}});"
...
esp_err_t otto_start_webserver(void) {
    ...
    config.max_uri_handlers = 24;  // Increased for UDP Drawing (5) + Gemini API (2) handlers
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        // ===== UDP Drawing handlers registration =====
        httpd_uri_t drawing_page_uri = {
            .uri = "/draw",
            .method = HTTP_GET,
            .handler = otto_drawing_page_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &drawing_page_uri);
        
        httpd_uri_t drawing_mode_uri = {
            .uri = "/drawing_mode",
            .method = HTTP_GET,
            .handler = otto_drawing_mode_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &drawing_mode_uri);
        
        httpd_uri_t drawing_clear_uri = {
            .uri = "/drawing_clear",
            .method = HTTP_GET,
            .handler = otto_drawing_clear_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &drawing_clear_uri);
        
        httpd_uri_t drawing_pixel_uri = {
            .uri = "/drawing_pixel",
            .method = HTTP_GET,
            .handler = otto_drawing_pixel_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &drawing_pixel_uri);
        
        httpd_uri_t drawing_status_uri = {
            .uri = "/drawing_status",
            .method = HTTP_GET,
            .handler = otto_drawing_status_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(server, &drawing_status_uri);
